...
// Get all categories for parent selection (excluding the current category)
$sql = "SELECT * FROM categories WHERE id != ? ORDER BY name";
$stmt_parents = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt_parents, "i", $category_id);
mysqli_stmt_execute($stmt_parents);
$parent_categories = mysqli_stmt_get_result($stmt_parents);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($slug)) {
        ...
        $check_slug_sql = "SELECT COUNT(*) FROM categories WHERE slug = ? AND id != ?";
        ...
        if ($slug_count > 0) {
            ...
            // Update category
            $sql = "UPDATE categories SET name = ?, slug = ?, description = ?, parent_id = ? WHERE id = ?";
            $stmt = mysqli_prepare($conn, $sql);
...
<head>
    ...
    <title>Edit Category - Cara Admin</title>
            ...
            <div class="form-container">
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="slug" id="slug" class="form-control" value="<?php echo htmlspecialchars($category['slug']); ?>" required>
                    </div>
                     ...
                     <div class="form-group">
                        ...
                        <select name="parent_id" id="parent_id" class="form-control">
                            ...
                            <?php while ($parent_category = mysqli_fetch_assoc($parent_categories)) { ?>
                                <option value="<?php echo $parent_category['id']; ?>" <?php echo $category['parent_id'] == $parent_category['id'] ? 'selected' : ''; ?> >
